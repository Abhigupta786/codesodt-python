...
def on_click(event):
    ...
    if text == "=":
        ...
        try:
            result=round(float(entry.get())**(1/2),3)
            entry.delete(0, tk.END)
            entry.insert(tk.END, str(result))
        except Exception as e:
            entry.delete(0, tk.END)
        ...
        try:
            result=round(float(entry.get())**(2),3)
            entry.delete(0, tk.END)
            entry.insert(tk.END, str(result))
        except Exception as e:
            entry.delete(0, tk.END)
        ...
        try:
            result=round(float(entry.get())**(3),3)
            entry.delete(0, tk.END)
            entry.insert(tk.END, str(result))
        except Exception as e:
            entry.delete(0, tk.END)
        ...
        try:
            result=round(float(entry.get())**(1/3),3)
            entry.delete(0, tk.END)
            entry.insert(tk.END, str(result))
        except Exception as e:
            entry.delete(0, tk.END)
...
root = tk.Tk()
root.title("Calc by Abhishek Gupta")
root.geometry("300x565")
root.configure(bg="#068FFF")
root.resizable(False, False)
entry = tk.Entry(root, font=("Helvetica", 30), justify="right",background="#068FFF",fg="#FFFFFF")
